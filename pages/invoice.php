...
if(!empty($id)){
	...
	if(!empty($order) && $user->id == $order->client && $order->pp_status == 1){
...
<div id="wrapper">
	...
	<div style="width:50%;float:left;">
		...
		<?php $state   = $objStates->getState($user->state);?>
		<?php echo $user->city.", ".$state->state_abbr; ?>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="tbl_repeat">
			...
			<?php foreach($items as $item){ ?>
				<tr>
					...
					<td class="ta_r">
						&dollar;<?php echo number_format($objBasket->itemTotal($item->price, $item->qty), 2); ?>
					</td>
				</tr>
			<?php } ?>
